...
namespace MsgPassingCommunication
{
  ...
  inline void Translater::postMessage(CsMessage^ csMsg)
  {
    Message msg = this->fromCsMessage(csMsg);
		std::cout << "\n  posting message: " + msg.command();
		if (msg.command() == "getInDirs")
			msg = pComm->getDirs(fromCsMessage(csMsg));
		if (msg.command() == "getInFiles")
			msg = pComm->getFiles(fromCsMessage(csMsg));
