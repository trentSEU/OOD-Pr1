...
int main()
{
  MockDb db;MockDbElem elem;
  elem.name("first elem");elem.description("test elem");
  db["first"] = elem;elem.name("second elem");
  db["second"] = elem;showDb(db);
  using Sptr = std::shared_ptr<AbstractXmlElement>;Sptr pDb = makeTaggedElement("db");
  pDb->addAttrib("type", "testDb");Sptr pDocElem = makeDocElement(pDb);
  XmlDocument xDoc(pDocElem);
  for (auto item : db){
    Sptr pRecord = makeTaggedElement("dbRecord");pDb->addChild(pRecord);
    Sptr pKey = makeTaggedElement("key", item.first);pRecord->addChild(pKey);
    Sptr pValue = makeTaggedElement("value");pRecord->addChild(pValue);
    Sptr pName = makeTaggedElement("name", item.second.name());pValue->addChild(pName);
    Sptr pDescrip = makeTaggedElement("description", item.second.description());pValue->addChild(pDescrip);
  }
  std::string Xml = xDoc.toString();MockDb newDb;XmlDocument newXDoc(Xml, XmlDocument::str);
  std::cout << newXDoc.toString();std::cout << "\n";
