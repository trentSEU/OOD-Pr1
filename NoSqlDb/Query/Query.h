...
namespace NoSqlDb
{
  ...
  public:
    ...
    Query& select(CallObj callObj);
		...
		template<typename CallObj>
		Query& select_and(CallObj callObj);

    Query& query_or(Query<P>& q);

		Query& query_ver(std::string ver);

		Query& query_and(Conditions<P>& conds);
  ...
  Query<P>& Query<P>::select(Conditions<P>& conds)
  {
	...
	/*----< returns keys for elements that match ver >---------------*/

	template<typename P>
	Query<P>& Query<P>::query_ver(std::string ver)
	{
		if (ver == "") return *this;
		Keys newKeys;
		for (auto key : keys_)
		{
			if (key.substr(key.find_last_of('.') + 1, key.size()) == ver)
	...
	template<typename P>
	Query<P>& Query<P>::query_and(Conditions<P>& conds)
	{
		Keys newKeys;
		for (auto item : db_)
		{
			if (find(keys_.begin(), keys_.end(), item.first) == keys_.end()) continue;
			conds.value(item.second);
			if (conds.match())
				newKeys.push_back(item.first);
		}
  ...
  */
  ...
  Query<P>& Query<P>::select(CallObj callObj)
  {
	...
	*/
	template<typename P>
	template<typename CallObj>
	Query<P>& Query<P>::select_and(CallObj callObj)
	{
		Keys newKeys;
		for (auto item : db_)
		{
