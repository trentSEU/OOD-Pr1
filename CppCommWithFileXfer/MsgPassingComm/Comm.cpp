...
const std::string storageRoot = "../../LocalStorage";
...
bool Sender::sendFile(Message msg)
{
  if (!msg.containsKey("file"))
    return false;
  std::string fileSpec = msg.value("sendPath");
  std::ifstream sendFile(fileSpec, std::ios::binary);
	if (!sendFile.good()) {
		std::cout << "not send good" << std::endl;
		return false;
	}
  while (true)
  {
...
public:
  ...
  */
  ...
  {
		int pos = msg.file().find_last_of('/');
    std::string fileName = msg.file().substr(pos + 1);
    std::string fileSpec = msg.value("receivePath") + "/" + msg.file();
    std::ofstream saveStream(fileSpec, std::ios::binary);
    if (!saveStream.good())
    {
			std::cout << "not receive good" << std::endl;
      return false;
    }
    while (true)
    {
      ...
      if (msg.contentLength() == 0)
		...
		msg.command("receiveEnd");						// end of receiving file
    saveStream.flush();
    saveStream.close();
		pQ_->enQ(msg);
    return true;
...
Message Comm::getDirs(Message msg) {
	...
	if (path != "")
	{
		...
		Dirs dirs = FileSystem::Directory::getDirectories(searchPath);
		size_t count = 0;
		for (auto item : dirs)
		{
			if (item != ".." && item != ".")
...
Message Comm::getFiles(Message msg) {
	std::string path = msg.value("path");
	if (path != "")
	{
		std::string searchPath = storageRoot;
		if (path != ".")
			searchPath = searchPath + "\\" + path;
		Files files = FileSystem::Directory::getFiles(searchPath);
		size_t count = 0;
		for (auto item : files)
		{
			std::string countStr = Utilities::Converter<size_t>::toString(++count);
